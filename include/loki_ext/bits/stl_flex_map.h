...
#ifndef _CPP_BITS_STL_FLEX_MAP_H
#define _CPP_BITS_STL_FLEX_MAP_H 1
...
public:
  ...
  friend class flex_map<_Key,_Tp,_Compare,_Alloc>;
  protected :
    _Compare comp;
...
private:
  ...
  _Rep_type _M_t;  // red-black tree representing flex_map
public:
  ...
  flex_map() : _M_t(_Compare(), allocator_type()) {}
  explicit flex_map(const _Compare& __comp,
               const allocator_type& __a = allocator_type())
    : _M_t(__comp, __a) {}

  template <class _InputIterator>
  flex_map(_InputIterator __first, _InputIterator __last)
    : _M_t(_Compare(), allocator_type())
    { _M_t.insert_unique(__first, __last); }

  template <class _InputIterator>
  flex_map(_InputIterator __first, _InputIterator __last, const _Compare& __comp,
      const allocator_type& __a = allocator_type())
    : _M_t(__comp, __a) { _M_t.insert_unique(__first, __last); }
  flex_map(const flex_map<_Key,_Tp,_Compare,_Alloc>& __x) : _M_t(__x._M_t) {}

  flex_map<_Key,_Tp,_Compare,_Alloc>&
  operator=(const flex_map<_Key, _Tp, _Compare, _Alloc>& __x)
  {
    _M_t = __x._M_t;
    return *this;
  }
  ...
  */
  ...
  void swap(flex_map<_Key,_Tp,_Compare,_Alloc>& __x) { _M_t.swap(__x._M_t); }
  ...
  */
  ...
  friend bool operator== (const flex_map<_K1, _T1, _C1, _A1>&,
                          const flex_map<_K1, _T1, _C1, _A1>&);
  template <class _K1, class _T1, class _C1, class _A1>
  friend bool operator< (const flex_map<_K1, _T1, _C1, _A1>&,
                         const flex_map<_K1, _T1, _C1, _A1>&);
};
...
inline bool operator==(const flex_map<_Key,_Tp,_Compare,_Alloc>& __x,
                       const flex_map<_Key,_Tp,_Compare,_Alloc>& __y) {
...
template <class _Key, class _Tp, class _Compare, class _Alloc>
inline void swap(flex_map<_Key,_Tp,_Compare,_Alloc>& __x,
                 flex_map<_Key,_Tp,_Compare,_Alloc>& __y) {
  __x.swap(__y);
}

} // namespace std

#endif /* _CPP_BITS_STL_FLEX_MAP_H */
