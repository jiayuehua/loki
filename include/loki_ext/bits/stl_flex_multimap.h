...
/** @file stl_flex_multimap.h
 *  This is an internal header file, included by other library headers.
 *  You should not attempt to use it directly.
 */

#ifndef __GLIBCPP_INTERNAL_FLEX_MULTIMAP_H
#define __GLIBCPP_INTERNAL_FLEX_MULTIMAP_H
...
inline bool operator==(const flex_multimap<_Key,_Tp,_Compare,_Alloc>& __x,
                       const flex_multimap<_Key,_Tp,_Compare,_Alloc>& __y);
 ...
 *  quickly retrieved through a key element. In contrast with a flex_map a
 *  flex_multimap can have multiple duplicate keys.
*/
template <class _Key, class _Tp, class _Compare, class _Alloc>
class flex_multimap
{
...
// typedefs:
  ...
  friend class flex_multimap<_Key,_Tp,_Compare,_Alloc>;
  protected:
    _Compare comp;
...
// allocation/deallocation
  ...
  flex_multimap() : _M_t(_Compare(), allocator_type()) { }
  explicit flex_multimap(const _Compare& __comp,
                    const allocator_type& __a = allocator_type())
    : _M_t(__comp, __a) { }

  template <class _InputIterator>
  flex_multimap(_InputIterator __first, _InputIterator __last)
  ...
  flex_multimap(const flex_multimap<_Key,_Tp,_Compare,_Alloc>& __x) : _M_t(__x._M_t) { }

  flex_multimap<_Key,_Tp,_Compare,_Alloc>&
  operator=(const flex_multimap<_Key,_Tp,_Compare,_Alloc>& __x) {
    _M_t = __x._M_t;
    return *this; 
  }
  ...
  */
  ...
  void swap(flex_multimap<_Key,_Tp,_Compare,_Alloc>& __x) { _M_t.swap(__x._M_t); }
  ...
  */
  ...
  size_type erase(const CompareType& __x) { return _M_t.erase(__x); }
  ...
  */
  template< class CompareType >
  inline const_iterator find(const CompareType& __x) const { return _M_t.find(__x); }
  ...
  */
  template< class CompareType >
  inline size_type count(const CompareType& __x) const { return _M_t.count(__x); }
  ...
  */
  template< class CompareType >
  iterator lower_bound(const CompareType& __x) {return _M_t.lower_bound(__x); }
  ...
  */
  template< class CompareType >
  iterator upper_bound(const CompareType& __x) {return _M_t.upper_bound(__x); }
  ...
  */
  ...
  friend bool operator== (const flex_multimap<_K1, _T1, _C1, _A1>&,
                          const flex_multimap<_K1, _T1, _C1, _A1>&);
  template <class _K1, class _T1, class _C1, class _A1>
  friend bool operator< (const flex_multimap<_K1, _T1, _C1, _A1>&,
                         const flex_multimap<_K1, _T1, _C1, _A1>&);
};
...
template <class _Key, class _Tp, class _Compare, class _Alloc>
inline void swap(flex_multimap<_Key,_Tp,_Compare,_Alloc>& __x,
                 flex_multimap<_Key,_Tp,_Compare,_Alloc>& __y) {
  __x.swap(__y);
}

} // namespace std

#endif /* __GLIBCPP_INTERNAL_FLEX_MULTIMAP_H */
