...
public:
	...
	bool valid() const throw()
	{
		...
		if (ErrorMessage.empty())
			return true;
		ATLTRACE(_T("The automation_vector is invalid due to "
	...
	throw(std::invalid_argument, std::runtime_error)
{
	...
	SAFEARRAY &Array = V_ISBYREF(&vSource) ? **vSource.pparray :
		*vSource.parray;
	if (Array.cDims != 1)
		throw std::invalid_argument("Only one-dimensional arrays "
			"are supported");
