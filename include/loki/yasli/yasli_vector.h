   ...
   private://-------ASSIGN IMPLEMENTATION
       ...
       void assign_impl(RanIt first, RanIt last, std::random_access_iterator_tag)
       {
           ...
           if (newSize >= size())
           {
               ...
               RanIt i = last - delta;
			   std::copy(first, i, ebo_.beg_);
               insert(end(), i, last);
           }
           else
           {
			   std::copy(first, last, ebo_.beg_);
               resize(newSize);
           }
           assert(size() == newSize);
       }
           ...
           FwdIterator first, FwdIterator last, std::forward_iterator_tag)
       {
           ...
           if (eos_ - end_ > count || reserve_inplace_nstd(size() + count)) // there's enough room
           {
               if (count > end_ - &*position)
               {
                   ...
                   T* const oldEnd = end_;
				   end_ = std::uninitialized_copy(i1, i2, end_);// jia should be uninitialized_copy//end_ = copy(i1, i2, end_);
